...
const BonusArea = (prop) => {
            ...
            if (!start.isBefore(end) && 1 == 2) {
                ...
                if (bonus.banaction) {
                    ...
                    if (parseInt(bonus.balancereq) < parseInt(loginToken.balance)) {
                        _msg2 = `${_msg2}<div><i class="fas fa-times fa-fw text-danger"></i> یا باید <span class="text-gold">لول شما  ${bonus.levelreq} یا بالاتر</span> باشد.</div><div><i class="fas fa-check fa-fw text-success"></i> یا موجودی اکانت شما بیش از <span class="text-gold">${doCurrency(bonus.balancereq)} تومان</span>  باشد.</div> ${_msg}</div>`;
                    } else {
                        _msg2 = `${_msg2}<div><i class="fas fa-times fa-fw text-danger"></i> یا باید <span class="text-gold">لول شما  ${bonus.levelreq} یا بالاتر</span> باشد.</div><div><i class="fas fa-times fa-fw text-danger"></i> یا موجودی اکانت شما بیش از <span class="text-gold">${doCurrency(bonus.balancereq)} تومان</span>  باشد.</div> ${_msg}</div>`;
                    }
    ...
    if (_mode == "gift") {
        if (_amount >= levelDataInfo[4].minAmount || (_amount >= levelDataInfo[4].minAmount / getRate && bonus.amount2)) {
            _mode = "gift3";
        } else if (_amount >= levelDataInfo[5].minAmount || (_amount >= levelDataInfo[5].minAmount / getRate && bonus.amount2)) {
            _mode = "gift2";
        } else {
            _mode = "gift1";
        }
        ...
        <List.Item>
            <List.Content floated="right">
                ...
                >
                    ...
                    {((bonus.status == "Pending" && start.isBefore(end) && !end.isBefore(expire)) || bonus.status == "Expired") && (
                        <>
                            انقضا در{" "}
                            <Moment fromNow locale="fa">
                                {expire}
                ...
                {bonus.status == "Done" && (
                    <>
                        <Button size="mini" color="green" compact disabled floated="right" className="rtl" style={{ opacity: 1, width: 140, marginRight: 10 }}>
                ...
                {((bonus.status == "Pending" && start.isBefore(end) && !end.isBefore(expire)) || bonus.status == "Expired") && (
                    <>
                        <Button animated="fade" size="mini" className="rtl" color="red" compact disabled floated="right" style={{ opacity: 1, width: 140, marginRight: 10 }}>
                            {doCurrency(_amount)} <small className="farsi">{_cur}</small>
                        </Button>
                    </>
                )}
                {bonus.status == "Pending" && !start.isBefore(end) && (
                    <>
                        ...
                        >
                            {doCurrency(_amount)} <small className="farsi">{_cur}</small>
                            {bonus.mode == "Gift" && lvlPercent < 100 && (
                                <>
                                    <br />
                                    {lvlPercent}%
                                </>
            ...
            <div style={{ padding: 10, overflow: "hidden" }} className={"rewardnam e"} mode={_mode.toLowerCase()}>
                <LevelIcon level={_lvl} mode={_mode.toLowerCase()} text={bonus.username} classinside={levelClassInside(_lvl - 1)} number={_lvl} width="36px" />
            </div>
        </List.Item>
    );
};
