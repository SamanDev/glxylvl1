...
const onSubmit = async (values, submitMethods, prop) => {
    ...
    if (prop.status == "Canceled") {
        ...
        if (values.mode == "CartToCart") {
            ...
            const res = await adminPostService(newValues, "visaGiftCode/manualCashoutSubmit", "");
            if (res.status == 200) {
                submitMethods.resetForm();
                prop.setFirstDone(false);
                prop.setFirstStatus("reload");
                                ...
                                <div className="onarea online1">
                                    {prop.status == "Done" ? (
                                        <>
                                            ...
                                            <FormikControl formik={formik} control="input" name="mode" labelcolor={prop.labelcolor} size={prop.size} />
                                            <Divider />
                                            <Button
                                                content={"انجام شد"}
                                                fluid
