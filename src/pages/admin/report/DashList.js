...
import FilterMode from "./DashFilter";
...
function Admin(prop) {
  ...
  const [startDate, setStartDate] = useState(addDays(new Date(), -12));
  ...
  const filteredItems = data
    ...
    .filter(
      (item) =>
        (dataSearch || item.status != "Canceled") &&
        (item.gateway != "AdminSystem" ||
          dataSearch == "AdminSystem" ||
          userSearch != "") &&
    ...
    return (
      <>
        ...
        >
          ...
          <Grid.Row>{prop.mychaty && <>{prop.mychaty}</>}</Grid.Row>
        </Grid>
      </>
    );
  }, [
